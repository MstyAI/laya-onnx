...
MAX_PROBABILITY_ERROR = 0.002
COMPARISON_FIELDS = ("case", "question", "options", "tokens", "expected", "actual")
...
def validate_report(report: dict, baseline_path: Path | None) -> None:
    for comparison in report["comparisons"]:
        if comparison["maxProbabilityError"] > MAX_PROBABILITY_ERROR:
            raise RuntimeError(f"ONNX probability parity failed: {comparison}")
    if baseline_path is None:
        return
    baseline = json.loads(baseline_path.read_text())
    for field in ("model", "revision"):
        if report[field] != baseline[field]:
            raise RuntimeError(f"parity baseline {field} changed")
    expected = baseline["comparisons"]
    actual = report["comparisons"]
    if len(actual) != len(expected):
        raise RuntimeError("parity baseline case count changed")
    for expected_case, actual_case in zip(expected, actual, strict=True):
        for field in COMPARISON_FIELDS:
            if actual_case[field] != expected_case[field]:
                raise RuntimeError(f"parity baseline changed {field}: {actual_case}")
...
def main() -> None:
    ...
    validate_report(report, args.baseline)
