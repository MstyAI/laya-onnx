...
func download(ctx context.Context, client *http.Client, source string, spec fileSpec, destination string) error {
	...
	hash := sha256.New()
	written, copyErr := io.Copy(io.MultiWriter(file, hash), io.LimitReader(response.Body, spec.bytes+1))
	closeErr := file.Close()
...
func installRuntime(ctx context.Context, client *http.Client, root string, spec runtimeSpec) error {
	archive := filepath.Join(root, ".onnxruntime-download")
	if err := download(ctx, client, spec.url, fileSpec{"onnxruntime", spec.sha256, spec.bytes}, archive); err != nil {
		return err
	}
