...
// Progress reports aggregate artifact download progress.
...
type Store struct {
	...
	Platform string
	// OnProgress receives bounded aggregate download updates.
...
// ErrNotPrepared means the verified model and runtime are not installed.
var ErrNotPrepared = errors.New("laya is not prepared")
...
// Resolve returns prepared paths without downloading anything. Preparation
// verifies hashes; resolution checks the completion marker and file sizes.
func (s *Store) Resolve(ctx context.Context) (Paths, error) {
	s.mu.Lock()
	defer s.mu.Unlock()
	if s.ready != nil {
		return *s.ready, nil
	...
	if !looksComplete(root, runtimeInfo, s.Platform) {
		return Paths{}, ErrNotPrepared
	}
	result := paths(root, runtimeInfo)
	s.ready = &result
	return result, nil
...
// DownloadSize returns the number of bytes required for a fresh preparation.
func (s *Store) DownloadSize() (int64, error) {
	s.mu.Lock()
	defer s.mu.Unlock()
	if err := s.applyDefaults(); err != nil {
		return 0, err
	...
	runtimeInfo, ok := runtimes[s.Platform]
	if !ok {
		return 0, fmt.Errorf("unsupported platform %s", s.Platform)
	}
	total := runtimeInfo.bytes
...
func (s *Store) install(ctx context.Context, root string, runtimeInfo runtimeSpec) error {
	...
	totalBytes := runtimeInfo.bytes
	for _, spec := range modelFiles {
		totalBytes += spec.bytes
	}
	report := newProgressReporter(s.OnProgress, totalBytes)
	var completedBytes int64
	for _, spec := range modelFiles {
		assetURL := *baseURL
		assetURL.Path = strings.TrimRight(baseURL.Path, "/") + "/" + spec.path
		if err := download(ctx, s.Client, assetURL.String(), spec, filepath.Join(stage, spec.path), report.forAsset(spec.path, completedBytes)); err != nil {
			return err
		}
		completedBytes += spec.bytes
	}
	if err := installRuntime(ctx, s.Client, stage, runtimeInfo, report.forAsset("onnxruntime", completedBytes)); err != nil {
		return err
	}
